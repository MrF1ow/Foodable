...
import {
  GroceryHeader,
  GroceryHeaderMin,
} from "@/components/page-specific/grocery/GroceryHeaders";
    ...
    <div className={`w-full h-full pl-0 md:pl-4 lg:pl-6 xl:pl-6 ${className}`}>
      <Card className="h-full flex flex-col bg-card-background rounded-lg">
        <CardHeader className="bg-primary text-[#202020] text-center rounded-lg">
          {isUser ? (
            <GroceryHeader additionalBackClick={additionalBackButtonClick} />
          ) : (
            <GroceryHeaderMin additionalBackClick={additionalBackButtonClick} />
          )}
        </CardHeader>
