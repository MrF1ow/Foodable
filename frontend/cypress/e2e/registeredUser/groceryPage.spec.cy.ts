...
describe("Rename grocery list", () => {
  const email = Cypress.env("USER_SIGN_IN_EMAIL");
  const password = Cypress.env("USER_SIGN_IN_PASSWORD");
  const originalTitle = `E2E List ${Date.now().toString().slice(-5)}`;
  const renamedTitle = `Renamed List ${Math.floor(Math.random() * 10000)}`;

  it("logs in, creates a list, renames it, and confirms the rename", () => {
    cy.clearCookies();
    cy.visit("/sign-in");
    cy.get('input[type="email"]').type(email);
    cy.get('input[type="password"]').type(password);
    ...
    cy.clickButton("grocery-header-dropdown");
    cy.clickButton("add-grocery-list");
    cy.typeText("new-list-title", originalTitle);
    cy.clickButton("select-category");
    cy.get('[role="option"]').contains("New Collection").click();
    cy.clickButton("submit-new-grocery-list");
    ...
    cy.clickButton("grocery-header-dropdown");
    cy.clickButton("edit-grocery-list");
    cy.typeText("list-title", renamedTitle);
    cy.clickButton("select-category");
    cy.get('[role="option"]').contains("New Collection").click();
    cy.clickButton("list-submit");
    cy.contains(renamedTitle, { timeout: 10000 }).should("be.visible");
